...
const Checkout = () => {
  ...
  const handleSubscription = async (event) => {
    ...
    while ((!razorPayKey || !subscription_id) && Date.now() - startTime < timeout) {
        await new Promise((resolve) => setTimeout(resolve, 200)); // check every 200ms
      }

    // after waiting → still not available → error
    if (!razorPayKey || !subscription_id) {
      toast.error("Still Wait...?, reload page");
      return;
    }
    ...
    const options = {
      ...
      prefill: {
        ...
        handler: async function (response) { //In Razorpay checkout, handler is a function that gets called automatically when the payment succeeds
    ...
    const paymentObject = new window.Razorpay(options); // it's script added in index.html
    paymentObject.open();
  };
      ...
      >
        ...
        <div className="w-80 h-[26rem] flex flex-col justify-center shadow-[0_0_10px_black] rounded-lg relative">
          ...
          <button
            ...
            // disabled={!razorPayKey || !subscription_id}
          > {!razorPayKey || !subscription_id ? "Wait..." : "Click to Pay"}
          </button>
